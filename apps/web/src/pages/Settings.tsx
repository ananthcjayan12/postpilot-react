        ...
        <section className="panel settings-card wide-card">
          ...
          <div className="settings-content">
            ...
            <div className="model-router">
              ...
              <label className="field"><span>Thumbnail generation</span><select className="input" value={defaults.aiRoutes.thumbnail} onChange={(e) => { const thumbnail = e.target.value as typeof defaults.aiRoutes.thumbnail; setDefaults({ ...defaults, aiRoutes: { ...defaults.aiRoutes, thumbnail, thumbnailResolution: thumbnailIs1KOnly(thumbnail) ? '1K' : defaults.aiRoutes.thumbnailResolution } }); }}><option value="gemini:gemini-3.1-flash-image">Gemini · 3.1 Flash Image</option><option value="gemini:gemini-3-pro-image">Gemini · 3 Pro Image (Nano Banana Pro)</option><option value="gemini:gemini-3.1-flash-lite-image">Gemini · 3.1 Flash Lite Image (1K only)</option><option value="gemini:gemini-2.5-flash-image">Gemini · 2.5 Flash Image (legacy, 1K only)</option><option value="openai:gpt-image-2.5-flare">OpenAI · GPT Image 2.5 Flare</option><option value="openai:gpt-image-2.5-sunburst">OpenAI · GPT Image 2.5 Sunburst</option></select><small>{defaults.aiRoutes.thumbnail === 'gemini:gemini-3-pro-image' ? 'Pro: detailed creative control, text rendering, and brand consistency. Supports 1K, 2K, and 4K.' : defaults.aiRoutes.thumbnail === 'gemini:gemini-3.1-flash-lite-image' ? 'Lite: faster, lower-cost generation at 1K. Choose Flash or Pro for demanding reference-image work.' : 'Choose the image model used to generate and regenerate thumbnails.'}</small></label>
              <label className="field"><span>Thumbnail resolution</span><select className="input" value={defaults.aiRoutes.thumbnailResolution} onChange={(e) => setDefaults({ ...defaults, aiRoutes: { ...defaults.aiRoutes, thumbnailResolution: e.target.value as typeof defaults.aiRoutes.thumbnailResolution } })}><option value="1K">1K · Fastest</option><option value="2K" disabled={thumbnailIs1KOnly(defaults.aiRoutes.thumbnail)}>2K · Detailed</option><option value="4K" disabled={thumbnailIs1KOnly(defaults.aiRoutes.thumbnail)}>4K · Maximum</option></select><small>Landscape: {defaults.aiRoutes.thumbnail.startsWith('openai:') ? ({ '1K': '1376×768', '2K': '2048×1152', '4K': '3840×2160' } as const)[defaults.aiRoutes.thumbnailResolution] : defaults.aiRoutes.thumbnail === 'gemini:gemini-2.5-flash-image' ? 'provider default (about 1K)' : `${defaults.aiRoutes.thumbnailResolution} output (dimensions vary by model)`}. Shorts automatically transpose to 9:16. Higher resolution increases latency, storage, and API cost.</small></label>
            </div>
          </div>
        </section>
        <section className="panel settings-card wide-card">
